...
class DetectionModel(nn.Module):
    def __init__(self, input_size, num_hiddens):
        ...
        self.rnn = nn.GRU(input_size, num_hiddens, num_layers=3, dropout=0.3)
    ...
    def forward(self, inputs, time_series_feature):
        # rnn训练访问时间序列，得到每个时间步骤的结果与时间序列 是LSTM的值 (state, _)
        enc_ouputs, state  = self.rnn(time_series_feature)
...
def train(train_path):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (features, labels, series) in enumerate(train_iter):
            ...
            if i % 5 == 0:
                log.info(f'epoch {epoch + 1}, loss {metric[0] / metric[2]:.5f}, train acc {metric[1] / metric[3]:.5f}')


        test_acc = evaluate_accuracy_gpu(net, test_iter)
        # animator.add(epoch + 1, (None, None, test_acc))

        log.info(f'epoch {epoch + 1}, loss {metric[0] / metric[2]:.5f}, train acc '
              f'{metric[1] / metric[3]:.5f}, test acc {test_acc:.5f}')
        # 计算F1_score, AUC, Recall
        # print(f'epoch {epoch + 1}')
        log.info(f'F1-Score:{f1_score(label_all, pred_all)}.5f,'
                 f'AUC:{roc_auc_score(label_all, pred_all)}.5f,'
                 f'Recall:{recall_score(label_all, pred_all)}.5f,'
                 f'Precision:{precision_score(label_all, pred_all)}.5f')

    torch.save(net.state_dict(), 'detection_model_using')

    endtime = datetime.datetime.now()

    log.info(f'训练耗时：{(endtime - starttime).seconds}')
...
if __name__ == '__main__':
    ...
    dataset_folder  = 'C:\\Users\\MSI\\Desktop\\'
